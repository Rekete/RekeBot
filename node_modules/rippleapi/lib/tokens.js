...
module.exports.getTokenInfo = async (token) => {
    try {
        if (!token || !validator.isMD5(token)) throw new Error('The token provided must be a valid MD5 hash. Try running createToken() to obtain one');
        let instance = axios.create({
            baseURL: 'https://ripple.moe/api/',
        ...
        const response = await instance.get('https://ripple.moe/api/v1/tokens/self');
        return response.data;
    } catch (e) {
        throw new Error(e);
    }
...
//         const response = await instance.get('https://ripple.moe/api/v1/tokens/self/delete');
//         return response.data;
//     } catch (e) {
//         throw new Error(e);
//     }
